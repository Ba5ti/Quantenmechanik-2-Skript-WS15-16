...
\section{Quantenmechanik}
	...
	\subsection{Quantenmechanik}
		...
		Da $\hat{A} = \hat{A}^\dagger$ selbstadjungiert ist, $\Rightarrow a_n \in \mathds{R}$ und es gilt $\braket{a_m|a_n} = \delta_{mn}$ (normiert) \grqq Strahl in H\grqq.
		
		Es gilt die Vollständigkeitsrelation:
			\begin{equation*}
				\sum_n \ket{a_n} \bra{a_n} = \mathds{1}
			\end{equation*}
